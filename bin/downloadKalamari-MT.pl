...
sub main{
  ...
  for my $s(@spreadsheet){
    logmsg "Downloading from spreadsheet $s";
...
sub downloadKalamari{
  ...
  while(<$fh>){
    ...
    push(@accessionToDownload, \%F);
    #my $fasta = downloadEntry(\%F, $settings);
    $download_counter++;
  }
  close $fh;

  logmsg "Initializing threads for downloading";
  my $Q = Thread::Queue->new(@accessionToDownload);
  my @thr;
  for(my $i=0;$i<$$settings{numcpus};$i++){
    $thr[$i] = threads->new(\&downloadWorker, $Q, $settings);
    $Q->enqueue(undef);
  }
  for(@thr){
    $_->join;
...
sub downloadWorker{
  my($Q, $settings) = @_;
  while(defined(my $fields = $Q->dequeue)){
...
sub downloadEntry{
  ...
  my $outfasta = "$dir/$acc.fasta";
  # If it exists, then skip the download
  if(-e $outfasta && -s $outfasta > 0){
    logmsg "  SKIP: found $outfasta already";
    return $outfasta;
  }

  # Get the esearch xml in place for at least one downstream query.
  # Save the xml as .xml.tmp and then .xml to help with 
...
sub usage{
  ...
  --numcpus  1  Download multiple assemblies at the same time.
                Warning: some downloads might be skipped if
                you go over the API calls per second limit.
